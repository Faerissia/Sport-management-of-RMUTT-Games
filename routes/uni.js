...
router.get("/", (req, res, next) => {
  dbConnection.query(
    "SELECT university.uniID, university.name, COUNT(faculty.uniID) AS count ,university.status FROM university LEFT JOIN faculty ON university.uniID = faculty.uniID GROUP BY university.uniID ORDER BY uniID asc",
    (err, rows) => {
      if (err) {
        req.flash("error", err);
...
router.get("/edit/(:uniID)", (req, res, next) => {
  ...
  dbConnection.query(
    "SELECT * FROM university WHERE uniID = " + uniID,
    (err, rows, fields) => {
      if (rows.length <= 0) {
...
router.post("/update/:uniID", (req, res, next) => {
  ...
  if (!errors) {
    ...
    // update query
    dbConnection.query(
      "UPDATE university SET ? WHERE uniID = " + uniID,
      form_data,
      (err, result) => {
        if (err) {
          req.flash("error", err);
...
router.get("/delete/(:uniID)", (req, res, next) => {
  ...
  dbConnection.query(
    "DELETE FROM university WHERE uniID = " + uniID,
    (err, result) => {
      if (err) {
        req.flash("error", err), res.redirect("/uni");
      } else {
